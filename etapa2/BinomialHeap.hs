...
emptyHeap = BinomialHeap { size = 0, trees = [] } -- heapu-l vid are size 0 si niciun arbore
...
-}
mergeTrees :: Ord p => [BinomialTree p k] -> [BinomialTree p k] -> [BinomialTree p k]  -- mergeTrees' recursive helper function 
mergeTrees ts1 ts2 = mergeTrees' (zipExtend EmptyTree EmptyTree ts1 ts2)  -- punem EmptyTree la lista mai scurta ca sa avem aceeasi lungime si facem zip 
  where 
    mergeTrees' [] = []  -- daca e Empty returnam Empty
    mergeTrees' [(t, EmptyTree)] = [t]  -- daca avem t, EmptyTree returnam t
    mergeTrees' [(EmptyTree, t)] = [t]  -- daca avem EmptyTree, t returnam t
    mergeTrees' ((t1, t2) : rest) = insertTree (attach t1 t2) (mergeTrees' rest)  -- luam prima pereche, ataseaza cei doi arbori, si inseram rezultatul 
